...
from utilities.constants import OK, NOT_FOUND, INTERNAL_SERVER_ERROR
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        return func.HttpResponse(json.dumps(list(results)[0]), status_code=OK)

    except Exception as e:
        logging.error(f"Error fetching session from Cosmos DB: {str(e)}")
        return func.HttpResponse(
            "Internal server error", status_code=INTERNAL_SERVER_ERROR
        )
